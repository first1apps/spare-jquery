...
( function( $ ) {
  ...
  $.spare = ( function() {
    ...
    */
    ...
    /** Calls the given function with the jQuery param */
    spare.$run = function( func ) {
      return func( $ );
    }
 ...
 * Checks against timing issue where a long click always refocuses.
 */
  ...
  jQuery.spare.plugin('focusToggler', 'init', function( $ ) {
    ...
    methods.init = function( ops ) {
      this.each( function() {
        ...
        $ele.off( _ns );
        $ele.on( 'mousedown touch focus', function() {
          lastInteraction = new Date().getTime();
        })
        $ele.on( 'click' + _ns, function() {
          $target.focusToggle('toggle', lastInteraction);
        });
